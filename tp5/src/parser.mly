...
expr:
 ...
 | FUNCTION pattern_expr_list { Ml_fun (fst $2) , List.fold_left (fun res a -> StrSet.union res a) (List.hd (snd $2)) (List.tl (snd $2)) }
 | application { List.fold_left (fun res a -> Ml_app(res, a)) (List.hd (fst $1)) (List.tl (fst $1)) , List.fold_left (fun res a -> StrSet.union res a) (List.hd (snd $1)) (List.tl (snd $1)) }
 | LET IDENT  EQUAL expr IN expr { Ml_let($2, fst $4, fst $6) , StrSet.union (snd $4) (StrSet.remove $2 (snd $6))  }
 | LET REC IDENT  EQUAL expr IN expr { Ml_letrec($3, failwith "let rec type expected", fst $5, fst $7) ,  StrSet.union (StrSet.remove $3 (snd $5)) (StrSet.remove $3 (snd $7)) }
...
pattern_expr_list:
 | pattern ARROW expr pattern_expr_list_next { (fst $1, fst $3) :: fst $4 , (StrSet.diff (snd $1) (snd $3)) :: (snd $4)  }

pattern_expr_list_next:
 | ALTERNATIVE pattern ARROW expr pattern_expr_list_next { (fst $2, fst $4) :: fst $5, (StrSet.diff (snd $2) (snd $4)) :: (snd $5) }
 | %prec NO_ALTERNATIVE { [] , [] }

application:
 | simple_expr_or_parenthesized_expr simple_expr_or_parenthesized_expr application_next { fst $1 :: fst $2 :: fst $3, snd $1 :: snd $2 :: snd $3 }
...
application_next:
 | simple_expr_or_parenthesized_expr application_next { fst $1 :: fst $2 , snd $1 :: snd $2 }
 | { [] , [] }
