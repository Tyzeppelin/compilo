...
main:
 ...
 | def END_OF_EXPRESSION { Ml_def $1 }
 | error {
 ...
 def:
 | LET IDENT EQUAL expr { Ml_def_let($2, $4) }
 | LET REC IDENT EQUAL expr { Ml_def_rec($3, $5) }

expr:
| simple_or_parenthesized_expr { $1 }
| FST expr { Ml_unop(Ml_fst, $2) }
| SND expr { Ml_unop(Ml_snd, $2) }
| expr CONS expr { Ml_cons($1, $3) }
| LEFT_BRACKET expr COMMA next_or_end_arr { Ml_cons($2, $4) }
...
| app { List.fold_left ( fun res a -> Ml_app(res, a)) (List.hd $1) (List.tl $1) }
| LET IDENT EQUAL expr IN expr { Ml_let($2, $4, $6) }
| LET REC IDENT EQUAL expr IN expr { Ml_letrec($3, $5, $7) }

simple_or_parenthesized_expr:
 | simple_expr { $1 }
...
next_or_end_arr:
 | expr COMMA next_or_end_arr { Ml_cons($1, $3) }
 | RIGHT_BRACKET { Ml_nil }

app:
 | simple_or_parenthesized_expr simple_or_parenthesized_expr app_next { $1 :: $2 :: $3 }

app_next:
 | simple_or_parenthesized_expr app_next { $1 :: $2 }
 |  { [] }
